...
export const Order = (props: any) => {
    ...
    return (
      ...
      >
        <View style={styles.orderInfo}>
          ...
          <View
            style={{
              backgroundColor: BackgroundOrder(item?.status),
              padding: 8,
              borderRadius: 8,
            }}
          >
            <AppText style={{color: ColorTxtOrder(item?.status)}}>
              {TxtOrderStatus(item?.status)}
            </AppText>
          </View>
        </View>
        <IconDots width={Device.width - Spacing.width70} />
        <View style={{marginVertical: Spacing.height10}}>
          <AppText style={styles.numItemTxt}>
            {item?.orderDetails?.length} {t('item')}
          </AppText>
          <View style={styles.productContainerInfo}>
            ...
            <View>
              <AppText style={styles.orderTotalTitle}>
                {t('orderTotal')}
              </AppText>
  ...
  const renderItemHeader = ({item, index}) => {
    return (
      <TouchableOpacity
        style={
          index === numTab ? styles.viewItemHeaderSelect : styles.viewItemHeader
        }
        onPress={() => {
          setTimeout(() => {
            setNumTab(index);
      ...
      >
        <AppText
          style={{color: index === numTab ? Colors.white : Colors.black}}
        >
          {item?.name}
        </AppText>
      </TouchableOpacity>
        ...
        <View style={{height: Device.height - Spacing.height200}}>
          ...
          >
            <TouchableOpacity
              style={{
                ...styles.btnHeader,
                backgroundColor: numUnConfimred
                  ? Colors.white
                  : Colors.colorMain2,
              }}
              onPress={() => {
            ...
            >
              <AppText
                style={{color: !numUnConfimred ? Colors.white : Colors.black}}
              >
                {t('UNPAID')}
              </AppText>
            </TouchableOpacity>
          ...
          <View style={{height: Device.height - Spacing.height250}}>
            <FlatList
              ...
              keyExtractor={(item, index) =>
                `list_image_big_${index.toString()}`
              }
              ref={refFlatList}
            ...
            ListEmptyComponent={() => {
              return (
                ...
                >
                  <IconOrderBig />
                  <AppText
                    style={{color: Colors.gray, marginTop: Spacing.width8}}
                  >
                    {t('notOrder')}
                  </AppText>
                </View>
  ...
  return (
    <View style={styles.container}>
      <HeaderScreen
        name={t('order')}
        goToBack={() => navigate(SCREEN_ROUTE.PROFILE_PAGE)}
      />
      <FlatList
        data={DataTitleOrder}
