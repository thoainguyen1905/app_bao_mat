    ...
    return(
        <TouchableOpacity style={styles.container} onPress={()=>{onSelect(item)}}>
            ...
            <View style={styles.containerView}>
                <AppImage uri={(typeof item?.product?.imageUrl)==='string'?`[${item?.product?.imageUrl}][0]`:item?.product?.imageUrl[0]} style={styles.image} resizeMode={"contain"} />
                <View style={{width:'68%',marginLeft:8}}>
                    ...
                    <AppText style={styles.txt_price}>{renderPrice(item?.product?.price)}</AppText>
