...
export function useModel(props: any) {
  ...
  useEffect(() => {
    if (colorSelect) {
      let newData = data?.productDetails.filter(
        (elm: any) => elm?.size.length !== 0,
      );
      let newSizes: any = newData
        ? newData.filter((elm: any) => elm.color === colorSelect?.color)
        : [];
      setSizes(newSizes);
  ...
  useEffect(() => {
    if (sizeSelect) {
      ...
      let newColors: any = newData.filter(
        (elm: any) => elm.size === sizeSelect?.size,
      );
      setColors(newColors);
